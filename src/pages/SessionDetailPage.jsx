...
export default function SessionDetailPage() {
  ...
  const renderAnswer = (answer) => {
  // normalize poll id
  const pollId = Number(answer.pollId || answer.poll_id);

  const poll = polls.find((p) => Number(p.id) === pollId);
  if (!poll) return `❓ Unknown poll (ID ${pollId})`;

  if (poll.type === "open-ended") {
    return `${poll.question}: ${answer.text}`;
  }

  const option = poll.options?.find((o) => Number(o.id) === Number(answer.optionId || answer.option_id));
  return `${poll.question}: ${option ? option.text : "❓ Unknown option"}`;
};
